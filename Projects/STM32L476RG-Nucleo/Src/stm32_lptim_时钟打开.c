...
void stm32_lptim_init(void)
{
    ...
    */
    ...
    __HAL_LPTIM_ENABLE_IT(&LptimHandle, LPTIM_IT_CMPM);//中断使能必须在LPTIM禁能时设置
...
*\Parameter     cmp
*\Return        void
*\Note          因为比较中断使能必须在lptim使能时才能设置 所以添加一个标记 记录是否在使用比较中断
                By Job 2018年6月7日
*\Log           2018.05.09    Ver 1.0    Job               
				创建函数。
***************************************************************************************************/
bool stm32_lptim_cmp_en(u16 cmp)
{
    ...
    __HAL_LPTIM_CLEAR_FLAG(&LptimHandle, LPTIM_FLAG_CMPM|LPTIM_FLAG_CMPOK);

    LpTimeFlag = TRUE;

    __HAL_LPTIM_COMPARE_SET(&LptimHandle, cmp);
    //必须等待置位才会生效 否则触发中断还是原来的CMP 等待时间大约3ms左右
    while (__HAL_LPTIM_GET_FLAG(&LptimHandle, LPTIM_FLAG_CMPOK) == RESET && delay++ < 0x3FFF * 5)
    {
    }
